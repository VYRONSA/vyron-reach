...
/**
 * The Assessment Centre — one entry point to every project's Engineering
 * Assessment. Each card links to /dev/assessment/[slug], which runs the
 * read-only Assessment Engine (lib/dev/assessment/) for that project.
 */
export default function DevAssessmentCentrePage() {
  ...
  return (
    <div>
      <DevPageHeader
        ...
        description="Observation, analysis, and recommendations for every project's engineering state — read-only, never modifies a project or repository."
      />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {!projects
          ...
          : projects.map(project => (
              <Link
                key={project.slug}
                href={`/dev/assessment/${project.slug}`}
              ...
              >
                ...
                <div className="mt-4 text-xs text-[var(--dev-accent)]">Run Engineering Assessment &rarr;</div>
              </Link>
            ))}
      </div>
    </div>
